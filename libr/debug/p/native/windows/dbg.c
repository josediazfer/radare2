...
static BOOL (WINAPI *w32_QueryThreadCycleTime)(HANDLE, PULONG64) = NULL;
static BOOL (WINAPI *w32_QueryProcessCycleTime)(HANDLE, PULONG64) = NULL;
// fpu access API
...
static void update_cpu_profile_info(RDebug *dbg)
{
	...
	if (dbg_w32->n_cpus <= 0) {
		...
		GetSystemInfo (&si);
		dbg_w32->n_cpus = si.dwNumberOfProcessors;
	} 
	if (dbg_w32->n_cpus <= 0) {
	...
	// cycle time for Idle
	idle_cycles = (_PSYSTEM_PROCESSOR_IDLE_CYCLE_TIME_INFORMATION)calloc (1, sizeof (*idle_cycles) * dbg_w32->n_cpus);
	if (!idle_cycles) {
	...
	if (!NT_SUCCESS (w32_NtQuerySystemInformation (SystemProcessorIdleCycleTimeInformation,
			idle_cycles,
			sizeof (_SYSTEM_PROCESSOR_IDLE_CYCLE_TIME_INFORMATION) * dbg_w32->n_cpus, NULL))) {
		r_sys_perror ("update_cpu_profile_info/NtQuerySystemInformation SystemProcessorIdleCycleInformation");
		goto err_update_cpu_profile_info;
	}
	cycles = 0;
	for (n = 0; n < dbg_w32->n_cpus; n++) {
		cycles += idle_cycles[n].CycleTime.QuadPart;
	}
	dbg_w32->profile.idle_cycles_delta = cycles - dbg_w32->profile.idle_cycles_value;
	dbg_w32->profile.idle_cycles_value = cycles;
	// cycle time for Interrupt
	intr_cycles = (_PSYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION)calloc (1, sizeof (*intr_cycles) * dbg_w32->n_cpus);
	if (!intr_cycles) {
	...
	if (!NT_SUCCESS (w32_NtQuerySystemInformation (SystemProcessorCycleTimeInformation,
			intr_cycles,
			sizeof (_SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION) * dbg_w32->n_cpus, NULL))) {
	...
	dbg_w32->profile.intr_cycles_delta = cycles - dbg_w32->profile.intr_cycles_value;
	dbg_w32->profile.intr_cycles_value = cycles;
	// cycle time for Dpc
	dpc_cycles = (_PSYSTEM_PROCESSOR_PERFORMANCE_INFORMATION)calloc (1, sizeof (*dpc_cycles) * dbg_w32->n_cpus);
	if (!dpc_cycles) {
	...
	if (!NT_SUCCESS (w32_NtQuerySystemInformation (SystemProcessorPerformanceInformation,
			dpc_cycles,
			sizeof (_SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION) * dbg_w32->n_cpus, NULL))) {
	...
	dbg_w32->profile.dpc_cycles_delta = cycles - dbg_w32->profile.dpc_cycles_value;
	dbg_w32->profile.dpc_cycles_value = cycles;
...
int w32_dbg_init(RDebug *dbg) {
	...
	dbg_w32->profile.proc_list =  r_list_newf ((RListFree)proc_profile_free);
...
static ut64 update_th_profile_info(RDebugW32 *dbg_w32, RDebugW32ThreadProfile *th) {
...
static RDebugW32ProcProfile* update_procs_profile_info(RDebug *dbg) {
	...
	DWORD tstamp;
	ut64 total_cycles = dbg_w32->profile.idle_cycles_delta +
			dbg_w32->profile.intr_cycles_delta +
			dbg_w32->profile.dpc_cycles_delta;
	...
	dbg_w32->profile.total_cycles = total_cycles;
...
static RDebugW32ProcProfile* update_ths_profile_info(RDebugW32 *dbg_w32, RDebugW32ProcProfile *proc) {
	...
	for (i = 0, th_proc_it = th_proc; i < th_proc_n; i++, th_proc_it++) {
		...
		th->tstamp = tstamp;
		update_th_profile_info (dbg_w32, th);
	}
